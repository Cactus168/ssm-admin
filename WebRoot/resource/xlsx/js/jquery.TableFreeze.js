...
$.fn.FrozenTable = function (iRowHead, iRowFoot, iColLeft) {//3,0,1
    ...
    if (iColLeft > 0 && oTable.width() > oDiv.width()) {
        ...
        oCloneDiv.css("background-color", "white");
...
$.fn.CloneTable = function (oSrcTable, iRowStart, iRowEnd, iColumnEnd) {
    ...
    for (var i = iRowStart; i < iRowEnd; i++) {
        ...
        var colCount = 0;
        var colNumber = 0;
        for (var j = -1; j < (iColumnEnd == -1 ? oldTr.find("td").length : iColumnEnd); j++) {
            //var oidTd = oldTr.find("td").eq(j);
			var oidTd = oldTr.find("td[c='"+j+"']");
			colNumber++;
        ...
        if (colNumber != 0) {
            ...
            for (var j = -1; j < colNumber; j++) {
				...
				if (typeof (colspan) != "undefined") {
					for(var n = i; n < iRowEnd; n++){
						var t_oldTr = oSrcTable.find("tr").eq(n);
						oidTd = t_oldTr.find("td[c='"+j+"']");
						colspan = oidTd.attr("colspan");
						if (typeof (colspan) == "undefined") {
							oidTd.text("");
							break;
						}
