...
public final class HttpHelper {
	...
	public static void addUserTOCookie(HttpServletResponse response, User user, int seconds) {
		try {
			user.setPassWord("");
			String userJson = URLEncoder.encode(CommonJsonUtils.toJsonString(user), "utf-8");
		...
		if (null != cookies) {
			for (Cookie cookie : cookies) {
				if (StringUtils.equals(USER_COOKIE, cookie.getName())) {
					try {
						return CommonJsonUtils.fromJsonString(URLDecoder.decode(cookie.getValue(), "utf-8"), User.class);
					} catch (UnsupportedEncodingException e) {
						e.printStackTrace();
					}
				}
